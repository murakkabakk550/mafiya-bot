...
TOKEN = "8831007425:AAHOo69nq2VqQhbNgX1T_C97HcBlGBk8310"
KUZATUVCHI = "javlonabdusamatov"
...
ROLLAR = {
    ...
    "Mafiya yordamchisi":     {"taraf": "mafiya", "rang": "🦹"},
    ...
    "O'lmas":                 {"taraf": "tinch",  "rang": "💎"},
    "Hamelion":               {"taraf": "tinch",  "rang": "🦎"},
    "Shoh":                   {"taraf": "tinch",  "rang": "👑"},
...
ROL_TAVSIF = {
    ...
    "Mafiya yordamchisi": "Sardor o'lsa, sen sardor bo'lasan.",
    ...
    "O'lmas": "Bir marta osib o'ldirishdan saqlanasan.",
    "Hamelion": "Mafiya/qotil o'ldirsa, o'rninga boshqa odam o'ladi.",
    "Shoh": "Ovozingiz 2 kishiga teng hisoblanadi.",
    "Bomber": "Tunda 3 kishini kuzatuv ostiga ol. O'lsang ular portlaydi.",
    "Qorbola": "Tunda kishilarni muzlatasan — mafiya o'ldira olmaydi.",
...
def rol_taqsimlash(n):
    # Asosiy rollar (har doim)
    rollar = ["Mafiya sardori", "Komissiyar", "Komissiyar yordamchisi",
              "Shifokor", "Jurnalist", "Qaysar", "Gadoyi", "O'lmas"]

    # 4-10 kishi: faqat asosiy + 1 mafiya
    if n <= 10:
        rollar = ["Mafiya sardori", "Komissiyar", "Komissiyar yordamchisi",
                  "Shifokor", "Jurnalist", "Qaysar", "Gadoyi", "O'lmas"]

    # 11-15 kishi: + qo'shimcha rollar
    elif n <= 15:
        rollar += ["Mafiya", "Mafiya yordamchisi", "Sehrgar", "Arvoh", "Hamelion"]

    # 16-20 kishi: + yana
    elif n <= 20:
        rollar += ["Mafiya", "Mafiya", "Mafiya yordamchisi",
                   "Sehrgar", "Arvoh", "Hamelion", "Shoh", "Qorbola", "Qotil"]

    # 21+ kishi: hammasi
    else:
        maf_soni = n // 7
        for _ in range(maf_soni - 1):
            rollar.append("Mafiya")
        rollar += ["Mafiya yordamchisi", "Sehrgar", "Arvoh", "Hamelion",
                   "Shoh", "Qorbola", "Qotil", "Bomber"]

    # Kerakli sondan ko'p bo'lsa kamaytir
    while len(rollar) > n:
        # Oxiridan qo'shimcha rollarni olib tashla
        for r in ["Bomber", "Qotil", "Qorbola", "Shoh", "Hamelion",
                  "Arvoh", "Sehrgar", "Mafiya yordamchisi", "Mafiya"]:
            if r in rollar and len(rollar) > n:
                rollar.remove(r)

    # Yetmasa tinch aholi qo'sh
    while len(rollar) < n:
...
class Oyinchi:
    def __init__(self, user_id, ism, username=None):
...
class GameState:
    ...
    def mafiyalar(self):
        return [o for o in self.tirik_oyinchilar()
                if o.rol in ("Mafiya", "Mafiya sardori", "Mafiya yordamchisi")]
    ...
    def golish_sharti(self):
        ...
        tinchlar = [o for o in tiriklar if ROLLAR.get(o.rol, {}).get("taraf") == "tinch"]
        qotil = self.qotil()
...
def kuzatuvchiga_xabar(context_games, chat_id):
    g = context_games.get(chat_id)
    if not g:
        return
    text = "👁️ *Barcha rollar (maxfiy):*\n\n"
    ...
    for o in g.oyinchilar:
        if o.username and o.username.lower() == KUZATUVCHI.lower():
            shaxsiy(o.user_id, text)
            return
...
def tugmalar(candidates, prefix, chat_id):
    ...
    for c in candidates:
        kb.add(InlineKeyboardButton(
            c.ism, callback_data=f"{prefix}|{chat_id}|{c.user_id}"
        ))
    return kb

# ======================== KOMANDALAR ========================

@bot.message_handler(commands=["start", "oyin", "oyun"])
def cmd_oyun(msg):
    if msg.chat.type == "private":
        bot.reply_to(msg, "Salom! Guruhga qo'shing va /oyin bosing.")
        return
    chat_id = msg.chat.id
    ...
    bot.reply_to(msg,
        ...
        "Qo'shilish: /qoshilish\n"
        "Kamida 4 kishi kerak.\n\n"
        f"👑 Admin: {msg.from_user.first_name}",
        parse_mode="Markdown")

@bot.message_handler(commands=["qoshilish", "qoshil"])
def cmd_qoshil(msg):
    chat_id = msg.chat.id
    if chat_id not in games:
        bot.reply_to(msg, "Avval /oyin yozing!")
        return
    g = games[chat_id]
    if g.boshlandi:
    ...
    if g.oyinchi(user.id):
        ...
        return
    g.oyinchilar.append(Oyinchi(user.id, user.first_name, user.username))
    bot.reply_to(msg, f"✅ {user.first_name} qo'shildi! Jami: {len(g.oyinchilar)} kishi")

@bot.message_handler(commands=["royxat", "royhatt"])
def cmd_royhat(msg):
    chat_id = msg.chat.id
    if chat_id not in games:
        return
...
def cmd_boshlash(msg):
    ...
    guruhga(chat_id,
        ...
        "Agar kelmasa — botga /start yuboring!\n\n"
        "Admin buyruqlari:\n"
        "▪️ /tun — tunni boshlash\n"
        "▪️ /tong — tun natijalarini e'lon qilish\n"
        "▪️ /ovoz — ovoz berish\n"
        "▪️ /holat — o'yinchilar holati\n"
        "▪️ /triloga — o'yinni tugatish")
    ...
    for o in g.oyinchilar:
        ...
        text = (f"🎭 *Sizning rolingiz:*\n\n"
                f"{rang} *{o.rol.upper()}*\n\n_{tavsif}_")
        if o.rol in ("Mafiya", "Mafiya sardori", "Mafiya yordamchisi") and len(mafiyalar) > 1:
    ...
    kuzatuvchiga_xabar(games, chat_id)
...
def cmd_tun(msg):
    ...
    if qb:
        n = len(tiriklar)
        muzlatish_soni = 2 if n <= 10 else (3 if n <= 15 else 5)
        candidates = [o for o in tiriklar if o != qb]
        shaxsiy(qb.user_id,
            f"❄️ *Qor Bola* — {muzlatish_soni} kishini muzlat:",
    ...
    if kom:
        ...
        shaxsiy(kom.user_id, "🔍 *Komissiyar* — Kimni tekshirasiz?",
    ...
    if bomb:
        n = len(tiriklar)
        kuzatish_soni = 2 if n <= 15 else 3
        candidates = [o for o in tiriklar if o != bomb]
        shaxsiy(bomb.user_id,
            f"💣 *Bomber* — {kuzatish_soni} kishini kuzatuv ostiga ol:",
    ...
    if gad:
        ...
        shaxsiy(gad.user_id, "🎒 *Gadoyi* — Qaysi uyga borasiz?",
...
def cmd_tong(msg):
    ...
    if "mafiya" in g.tun_harakatlari:
        ...
        if nishon and nishon.tirik:
            if nishon.muzlatilgan:
                xabarlar.append("❄️ Bir kishi muzlatilgan edi — mafiya o'ldira olmadi!")
            elif nishon.himoyalangan:
                xabarlar.append("💊 Bir kishi shifokor tomonidan saqlanib qoldi!")
            elif nishon.rol == "Hamelion":
                boshqalar = [o for o in g.tirik_oyinchilar() if o != nishon]
                if boshqalar:
                    alm = random.choice(boshqalar)
                    alm.tirik = False
                    xabarlar.append(f"🦎 Hamelion kuchi ishga tushdi — {alm.ism} o'ldi!")
            elif nishon.rol == "O'lmas" and not nishon.olmas_ishlatilgan:
                nishon.olmas_ishlatilgan = True
                xabarlar.append(f"💎 {nishon.ism} omon qoldi!")
            else:
                nishon.tirik = False
                xabarlar.append(f"💀 {nishon.ism} tunda o'ldirildi.")
                # Sardor o'lsa — yordamchisi sardor bo'ladi (yashirin)
                if nishon.rol == "Mafiya sardori":
                    yord = next((o for o in g.tirik_oyinchilar()
                                 if o.rol == "Mafiya yordamchisi"), None)
                    if yord:
                        yord.rol = "Mafiya sardori"
                        shaxsiy(yord.user_id, "😈 Sardor o'ldi. Endi siz sardorsiz!")
                if nishon.rol == "Komissiyar":
                    yord = next((o for o in g.tirik_oyinchilar()
                                 if o.rol == "Komissiyar yordamchisi"), None)
                    if yord:
                        yord.rol = "Komissiyar"
                        shaxsiy(yord.user_id, "🔍 Komissiyar o'ldi. Endi siz komissiyarsiz!")
        ...
        xabarlar.append("🌙 Tunda hech narsa bo'lmadi.")
    ...
    if "gadoyi" in g.tun_harakatlari:
        ...
        if gad and gad.tirik and nishon and not nishon.tirik:
            ...
            if maf_nishon == nishon.user_id:
                ...
                shaxsiy(gad.user_id, f"🎒 Borgan uyingiz o'ldirildi!\nMafiya: {maf_ismlar}")
    ...
    text += "\n\nMuhokama qiling! Tayyor bo'lgach /ovoz"
    g.faza = "kun"
    g.tun_harakatlari = {}
...
def cmd_holat(msg):
    chat_id = msg.chat.id
    if chat_id not in games:
        bot.reply_to(msg, "O'yin yo'q.")
        return
    g = games[chat_id]
    text = f"📊 *{g.kun}-kun | {g.faza}*\n\n" + tirik_royxat(g)
    bot.reply_to(msg, text, parse_mode="Markdown")

@bot.message_handler(commands=["triloga"])
def cmd_triloga(msg):
    chat_id = msg.chat.id
    if chat_id not in games:
        bot.reply_to(msg, "O'yin yo'q.")
...
def cmd_yordam(msg):
    bot.reply_to(msg,
        ...
        "/tong — Tong/tunni yakunlash (admin)\n"
        "/ovoz — Ovoz berish (admin)\n"
        "/holat — O'yin holati\n"
        "/triloga — O'yinni tugatish (admin)",
        parse_mode="Markdown")
...
def callback(call):
    ...
    if amal == "shifokor":
        ...
        bot.edit_message_text(f"😈 Nishon tanlandi ✅",
                              call.message.chat.id, call.message.message_id)
        ...
        bot.edit_message_text("🔍 Tekshirildi ✅",
                              call.message.chat.id, call.message.message_id)
        ...
        bot.edit_message_text("📰 Bilib oldingiz ✅",
                              call.message.chat.id, call.message.message_id)

    elif amal == "qotil":
        g.tun_harakatlari["qotil"] = nishon_id
        bot.edit_message_text("🗡️ Nishon tanlandi ✅",
                              call.message.chat.id, call.message.message_id)
        ...
        n = len(g.tirik_oyinchilar())
        max_son = 2 if n <= 10 else (3 if n <= 15 else 5)
        if "qorbola" not in g.tun_harakatlari:
            g.tun_harakatlari["qorbola"] = []
        lst = g.tun_harakatlari["qorbola"]
        if len(lst) < max_son and nishon_id not in lst:
            lst.append(nishon_id)
            nishon.muzlatilgan = True
            bot.answer_callback_query(call.id, f"❄️ {nishon.ism} muzlatildi! ({len(lst)}/{max_son})")
            if len(lst) >= max_son:
                bot.edit_message_text(f"❄️ {max_son} kishi muzlatildi ✅",
                                      call.message.chat.id, call.message.message_id)
            else:
                qb = g.oyinchi(user_id)
                candidates = [o for o in g.tirik_oyinchilar()
                              if o != qb and o.user_id not in lst]
                if candidates:
                    try:
        ...
        if bomb and len(lst) < max_son and nishon_id not in lst:
            lst.append(nishon_id)
            bomb.bomber_kuzatuvdagilar.append(nishon)
            bot.answer_callback_query(call.id, f"💣 {nishon.ism} kuzatuvda! ({len(lst)}/{max_son})")
            if len(lst) >= max_son:
                bot.edit_message_text(f"💣 {max_son} kishi kuzatuvda ✅",
                                      call.message.chat.id, call.message.message_id)
            else:
                candidates = [o for o in g.tirik_oyinchilar()
                              if o != bomb and o.user_id not in lst]
                if candidates:
                    try:
                        bot.edit_message_reply_markup(
                            call.message.chat.id, call.message.message_id,
        ...
        if not beruvchi or not beruvchi.tirik:
            ...
            return
        ovoz_soni = 2 if beruvchi.rol == "Shoh" else 1
...
def ovoz_yakunla(g):
    ...
    kuzatuvchiga_xabar(games, g.chat_id)
...
def osib_oldirish(g, nishon):
    if nishon.rol == "Qaysar" and not nishon.qaysar_chiqdi:
        ...
        return f"💪 {nishon.ism} — bu safar chiqmadi!"
    if nishon.rol == "O'lmas" and not nishon.olmas_ishlatilgan:
        nishon.olmas_ishlatilgan = True
        return f"💎 {nishon.ism} — omon qoldi!"
    nishon.tirik = False
    xabar = f"⚰️ {nishon.ism} osib o'ldirildi!"
    if nishon.bomber_kuzatuvdagilar:
        xabar += "\n💥 PORTLASH!"
        for k in nishon.bomber_kuzatuvdagilar:
            if k.tirik:
